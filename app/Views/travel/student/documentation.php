...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
            Ketua Tim: <span class="font-bold text-primary-600"><?= esc($member->name ?? $student->name ?? auth()->user()->username) ?></span>
            <span class="mx-2 text-slate-300">|</span>
            No Surat Tugas: <span class="font-bold text-slate-800"><?= esc($request->no_surat_tugas) ?></span>
        </p>
    </div>
...
<?php if (auth()->user()->inGroup('superadmin', 'verificator')): ?>
    <div class="alert-info mb-6 flex items-center gap-3">
        ...
        <p class="text-sm font-medium">Anda sedang melihat/mengubah dokumentasi sebagai <strong>Superadmin/Verifikator</strong>. Perubahan yang disimpan akan tercatat pada dokumentasi tim ini.</p>
    </div>
<?php endif; ?>
...
foreach ($completeness as $item) {
    ...
    if ($item->status === 'uploaded' || !empty($item->files)) $hasUploaded = true;
}

if ($hasRejected) {
    $memberStatus = 'rejected';
    $statusLabel = 'NEEDS REVISION';
    $statusClass = 'bg-rose-50 text-rose-600 border border-rose-100 shadow-sm';
...
<div class="mb-6 space-y-3">
    <div class="card p-4 flex flex-wrap items-center justify-between gap-4 border-l-4 <?= $memberStatus === 'rejected' ? 'border-l-rose-500' : ($memberStatus === 'verified' ? 'border-l-emerald-500' : 'border-l-slate-300') ?>">
        ...
        <?php if ($memberStatus === 'rejected'): ?>
            <div class="flex-1 min-w-[300px] lg:max-w-md p-3 bg-white/60 rounded-xl border border-rose-100 flex items-start gap-2.5">
                <i data-lucide="alert-circle" class="w-4 h-4 text-rose-500 mt-0.5"></i>
                <div>
                    <p class="text-[9px] font-black text-rose-600 uppercase tracking-widest leading-none mb-1">Pesan Revisi</p>
...
<form action="<?= base_url('travel/student/' . $request->id . '/documentation') ?>" method="POST" enctype="multipart/form-data">
    <?= csrf_field() ?>
            ...
            <div class="card p-0 border-none shadow-premium bg-white overflow-hidden">
                <div class="bg-primary-500 px-6 py-4 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-slate-100 font-medium">Berikan ringkasan pelaksanaan kegiatan tim mahasiswa</p>
                        </div>
                    </div>
                </div>
                <div class="p-6 bg-slate-50/30">
                    <textarea
                        ...
                        placeholder="Contoh: Tim mahasiswa telah menyelesaikan seluruh agenda kegiatan..."
                        <?= ($request->status === 'completed' && !auth()->user()->inGroup('superadmin')) ? 'readonly' : '' ?>><?= esc((string)($member->report_narrative ?? '')) ?></textarea>
                    <div class="mt-3 flex items-start gap-2 text-[10px] text-slate-500 italic px-1">
                        ...
                        <span>Narasi ini akan disertakan dalam ringkasan laporan dokumentasi perjalanan mahasiswa.</span>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 space-y-4">
                    <?php if (empty($completeness)): ?>
                        <div class="flex flex-col items-center justify-center py-12 px-4 text-center">
                            ...
                            <p class="text-[11px] text-slate-500 mt-1">Daftar berkas akan muncul setelah data kelengkapan disiapkan oleh petugas.</p>
                        </div>
                    <?php endif; ?>
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-1">
                                                ...
                                                <?php if ($item->status === 'rejected'): ?>
                                                    <span class="px-1.5 py-0.5 rounded bg-rose-100 text-rose-600 text-[8px] font-black uppercase">REVISION</span>
                                                <?php elseif ($item->status === 'verified'): ?>
                                                    <span class="px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-600 text-[8px] font-black uppercase">APPROVED</span>
                                                <?php endif; ?>
                                            </div>
                                            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-tight"><?= esc($item->remark ?: 'Wajib diunggah (Nota/Tiket/Bukti Fisik)') ?></p>
                                        </div>
                                    </div>
                                            ...
                                            <div class="grid grid-cols-1 gap-2">
                                                <?php foreach ($item->files as $file): ?>
                                                    <div class="flex items-center justify-between p-2.5 rounded-xl bg-slate-50/50 border border-slate-100 group/file hover:bg-white hover:border-blue-100 transition-all">
                                                        ...
                                                        <?php if (!($request->status === 'completed' && !auth()->user()->inGroup('superadmin'))): ?>
                                                            <button type="button"
...
<script>
    function updateFileNameDisplay(input, displayId) {
        ...
        if (files.length === 0) {
            display.innerText = 'PILIH BERKAS';
    ...
    async function deleteFile(fileId, button) {
        ...
        try {
            const response = await axios.delete('<?= base_url('travel/student/file') ?>/' + fileId, {
                headers: {
