...
<div class="card max-w-lg mx-auto">
    ...
    <!-- Form -->
    <form action="<?= base_url('admin/tariffs/' . $tariff->id . '/update') ?>" method="post" class="space-y-5">
        <?= csrf_field() ?>

        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                <label class="form-label" for="province">Provinsi <span class="text-red-500">*</span></label>
                <select id="province" name="province" class="input-control" data-old-value="<?= old('province', $tariff->province) ?>" required>
            ...
            <div>
                ...
                <select id="city" name="city" class="input-control" data-old-value="<?= old('city', $tariff->city) ?>">
        ...
        <div>
            ...
            <select id="tingkat_biaya" name="tingkat_biaya" class="input-control" required>
                ...
                <option value="A" <?= old('tingkat_biaya', $tariff->tingkat_biaya) === 'A' ? 'selected' : '' ?>>Tingkat A</option>
                <option value="B" <?= old('tingkat_biaya', $tariff->tingkat_biaya) === 'B' ? 'selected' : '' ?>>Tingkat B</option>
                <option value="C" <?= old('tingkat_biaya', $tariff->tingkat_biaya) === 'C' ? 'selected' : '' ?>>Tingkat C</option>
                <option value="D" <?= old('tingkat_biaya', $tariff->tingkat_biaya) === 'D' ? 'selected' : '' ?>>Tingkat D</option>
            </select>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="uang_harian" name="uang_harian" value="<?= old('uang_harian', $tariff->uang_harian) ?>" class="input-control pl-10" placeholder="0" required>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="uang_representasi" name="uang_representasi" value="<?= old('uang_representasi', $tariff->uang_representasi) ?>" class="input-control pl-10" placeholder="0" required>
                </div>
            </div>

            <div>
                <label class="form-label" for="jenis_penginapan">Jenis Penginapan <span class="text-red-500">*</span></label>
                <input type="text" id="jenis_penginapan" name="jenis_penginapan" value="<?= old('jenis_penginapan', $tariff->jenis_penginapan) ?>" class="input-control" placeholder="Contoh: Hotel Bintang 3 / Villa" required>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" id="penginapan" name="penginapan" value="<?= old('penginapan', $tariff->penginapan) ?>" class="input-control pl-10" placeholder="0" required>
                </div>
            </div>
        ...
        <div>
            ...
            <input type="number" id="tahun_berlaku" name="tahun_berlaku" value="<?= old('tahun_berlaku', $tariff->tahun_berlaku) ?>" class="input-control" placeholder="YYYY" required>
        </div>
        ...
        <div class="flex items-center justify-between rounded-lg border border-surface-200 p-4">
            ...
            <label class="relative inline-flex cursor-pointer items-center">
                <!-- Checkbox -->
                <input type="checkbox" name="is_active" value="1" class="peer sr-only" <?= old('is_active', $tariff->is_active) == '1' ? 'checked' : '' ?>>
